...
func RunVegeta(ctx context.Context, requestDicts []map[string]interface{}, testName string, rps int) error {
	startTime := time.Now()
	...
	attacker := vegeta.NewAttacker(vegeta.Client(client), vegeta.Timeout(120*time.Second))
